...
import MobileLoginForm from "../components/auth/MobileLogin";
import MobileSignUpForm from "../components/auth/MobileSignUp";
...
export default function Auth() {
  ...
  const renderMobileContent = () => (
    <div className="relative z-30 w-full max-w-md  rounded-lg p-5">
      {isSignUp ? (
        <MobileSignUpForm setShowServey={setShowServey} />
      ) : (
        <MobileLoginForm setChangePassword={setChangePassword} />
      )}
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden">
      <div
        className="absolute top-3 left-3 w-8 h-8 md:w-10 md:h-10 flex items-center justify-center bg-gray-800 text-white rounded-full shadow-lg cursor-pointer hover:bg-gray-700 transition-colors z-30"
        onClick={() => navigate("/")}
      >
